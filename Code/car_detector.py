...
#Select model
model = 3 # is not recommended (takes very long)
...
with detection_graph.as_default():
  with tf.Session(graph=detection_graph) as sess:
    ...
    while(True):
        ...
        image = Image.open('/tensorflow/models/research/object_detection/test_images/picture.jpg')
        ...
        count = 0
        for i in range(100):
            if scores is None or final_score[i] > 0.4: #if the probability of eing a car is > 0.5
                if classes[0][i] == 3:
